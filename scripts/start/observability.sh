#!/bin/bash
# Start observability (Start Phase - Deployment)
# PURPOSE: Starts observability services/containers.
...
set_script_log "start_observability"

log_info "Starting observability..."
...
case "observability" in
  ...
  registry)
    ...
    if ! docker ps -a --format '{{.Names}}' | grep -q "oss-registry"; then
      ...
      bash "$REPO_ROOT/scripts/infra/registry.sh"
    else
...
log_info "RESULT: observability started"
exit 0
